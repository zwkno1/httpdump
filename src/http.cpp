...
std::string HttpMessage::toString() const
{
	...
	if(isRequest)
	{
		ss << httpMethodStr(method) << " " << url
			<< " HTTP/" << version.major << "." << version.minor << CRLF;
		for(auto & h : header)
		{
	...
	else
	{
		ss << "HTTP/" << version.major << "." << version.minor
			<< " " << statusCode << " " << httpStatusCodeStr(statusCode) << CRLF;
		for(auto & h : header)
		{
			ss << h.first << ":" << h.second << CRLF;
		}
...
const char * httpMethodStr(std::uint8_t method)
{
